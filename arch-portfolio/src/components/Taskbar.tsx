...
const Taskbar: React.FC<TaskbarProps> = ({ apps, onAppClick }) => {
  return (
    <div
      className="absolute bottom-4 right-4 z-50 flex items-center gap-2 backdrop-blur-md rounded-lg text-white shadow-lg text-sm font-sans select-none border border-white/10 transition-all duration-300"
      style={{ minHeight: '40px', padding: apps.length === 0 ? '4px 12px' : '8px' }}
    >
      {apps.length === 0 ? (
